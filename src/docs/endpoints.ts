...
export const endpoints: EndpointDoc[] = [
  ...
  // ---------------- MESSAGES ----------------
  {
    ...
    params: [
      { name: "url", type: "string", required: true, description: "Encrypted media URL from `imageMessage.url` (or `videoMessage.url`, etc.) in the incoming webhook payload." },
      { name: "mediaKey", type: "string", required: true, description: "Encryption key from `imageMessage.mediaKey` in the webhook payload." },
      { name: "mimetype", type: "string", required: true, description: "MIME type of the media (e.g. `image/jpeg`, `video/mp4`, `audio/ogg`)." },
      { name: "type", type: "string", required: true, description: "Media type: `image`, `video`, `audio`, `document`, `sticker`." },
      { name: "directPath", type: "string", required: true, description: "Direct path from `imageMessage.directPath` in the webhook payload." },
      { name: "fileEncSha256", type: "string", required: true, description: "Encrypted file SHA-256 from `imageMessage.fileEncSha256` in the webhook payload." },
      { name: "fileLength", type: "string", required: true, description: "File length in bytes from `imageMessage.fileLength` in the webhook payload." },
    ],
    exampleBody: {
      url: "PASTE_imageMessage.url_HERE",
      mediaKey: "PASTE_imageMessage.mediaKey_HERE",
      mimetype: "image/jpeg",
      type: "image",
      directPath: "PASTE_imageMessage.directPath_HERE",
      fileEncSha256: "PASTE_imageMessage.fileEncSha256_HERE",
      fileLength: "PASTE_imageMessage.fileLength_HERE",
    },
